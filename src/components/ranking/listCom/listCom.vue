<template>
  <div id="wrap">
      ...
      <div class="info_box">
        <div class="gn">
          <div class="img" style="width:125px;height:125px;margin-right: 10px;">
            <img :src="logo(data.logo)" />
            <!--<img :src="data.logo || NewDieT + data.mid + NewDieH" />-->
          </div>
          <div class="album">
            <h3 class="son_h3">{{songTitle(data.dissname)}}</h3>
            <div class="use">
              <img :src="songer(data.headurl)" alt="">
              <span>{{name(data.nickname)}}</span>
            </div>
            <p>{{visitnum(data.visitnum)}}</p>
          </div>
        </div>
        <div class="bf">
    ...
    <div class="main" :style="HeightStyle">
      <div class="count"><span>{{listLeng(data.songlist)}}</span><span @click="PushCollectsong()">收藏</span></div>
